...
function RecipeList(props) {
  const RecipeListItemArr = props.recipes.map((recipe) => (
    <RecipeListItem
      ...
      sugarPerServing={
        recipe.recipe.totalNutrients.SUGAR.quantity / recipe.recipe.yield
      }
      proteinPerServing={
        recipe.recipe.totalNutrients.PROCNT.quantity / recipe.recipe.yield
      }
      fiberPerServing={
        recipe.recipe.totalNutrients.FIBTG.quantity / recipe.recipe.yield
      }
      potassiumPerServing={
        recipe.recipe.totalNutrients.K.quantity / recipe.recipe.yield / 1000
      }
      vitaminAPerServing={
        recipe.recipe.totalNutrients.VITA_RAE.quantity /
        recipe.recipe.yield /
        1000000
      }
      vitaminCPerServing={
        recipe.recipe.totalNutrients.VITC.quantity / recipe.recipe.yield / 1000
      }
      calciumPerServing={parseFloat(
        (
          recipe.recipe.totalNutrients.CA.quantity /
          recipe.recipe.yield /
          1000
        ).toFixed(5)
      )}
      ironPerServing={
        recipe.recipe.totalNutrients.FE.quantity / recipe.recipe.yield / 1000
      }
